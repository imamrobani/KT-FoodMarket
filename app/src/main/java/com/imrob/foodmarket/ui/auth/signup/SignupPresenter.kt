...
import com.imrob.foodmarket.network.HttpClient
...
class SignupPresenter(private val view: SignupContract.View) : SignupContract.Presenter {
    ...
    override fun submitRegister(registerRequest: RegisterRequest, viewParms: View) {
        view.showLoading()
        val disposable =
            HttpClient.getInstance().getApi()!!.register(
    ...
    override fun submitPhotoRegister(filePath: Uri, viewParms: View) {
        ...
        val profileImageRequestBody =
            profileImageFile.asRequestBody("multipart/form-data".toMediaTypeOrNull())
        ...
        val disposable =
            HttpClient.getInstance().getApi()!!.registerPhoto(profileImageParams)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread()).subscribe(
                    {
                        view.dismissLoading()
